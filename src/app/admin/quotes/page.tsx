        ...
        <>
          ...
          <div className="admin-desktop-only" style={{ background: "#111", border: "1px solid #222", borderRadius: "4px", overflow: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
              ...
              <tbody>
                {filtered.map((q) => {
                  const orphan = !q.inquiry_id;
                  const customerLabel = orphan ? "Customer deleted" : (q.inquiries?.name ?? "—");
                  return (
                  <tr key={q.id} style={{ borderBottom: "1px solid #1a1a1a", transition: "background 0.15s", opacity: orphan ? 0.7 : 1 }} onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(255,255,255,0.02)")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                    <td style={{ padding: "0.75rem 1rem" }}>
                      {orphan ? (
                        <span style={{ color: "rgba(255,255,255,0.5)", fontWeight: 600 }}>{q.quote_number}</span>
                      ) : (
                        <Link href={`/admin/inquiries/${q.inquiry_id}/quote?quote_id=${q.id}`} style={{ color: "#F7941D", textDecoration: "none", fontWeight: 600 }}>{q.quote_number}</Link>
                      )}
                    </td>
                    <td style={{ padding: "0.75rem 1rem", color: orphan ? "rgba(255,255,255,0.4)" : "#fff", whiteSpace: "nowrap", fontStyle: orphan ? "italic" : "normal" }}>{customerLabel}</td>
                    <td style={{ padding: "0.75rem 1rem", color: "rgba(255,255,255,0.5)" }}>{orphan ? "—" : (q.inquiries?.email ?? "—")}</td>
                    <td style={{ padding: "0.75rem 1rem", color: "rgba(255,255,255,0.5)", whiteSpace: "nowrap" }}>{orphan ? "—" : (q.inquiries?.service ?? "—")}</td>
          ...
          <div className="admin-mobile-only" style={{ display: "none", flexDirection: "column", gap: "0.75rem" }}>
            {filtered.map((q) => {
              const orphan = !q.inquiry_id;
              const card = (
                <div style={{ background: "#111", border: "1px solid #222", borderRadius: "4px", padding: "1rem", transition: "border-color 0.2s", opacity: orphan ? 0.7 : 1 }}>
                  {/* Top row: Quote # + Total */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                    <span style={{ color: orphan ? "rgba(255,255,255,0.5)" : "#F7941D", fontWeight: 700, fontSize: "14px" }}>{q.quote_number}</span>
                    <span style={{ color: "#F7941D", fontWeight: 700, fontSize: "16px" }}>{formatCurrency(q.total)}</span>
                  </div>
                  {/* Customer */}
                  <p style={{ color: orphan ? "rgba(255,255,255,0.4)" : "#fff", fontSize: "15px", fontWeight: 600, margin: "0 0 0.2rem", fontStyle: orphan ? "italic" : "normal" }}>
                    {orphan ? "Customer deleted" : (q.inquiries?.name ?? "—")}
                  </p>
                  {!orphan && (
                    <>
              ...
              return orphan ? (
                ...
                <Link key={q.id} href={`/admin/inquiries/${q.inquiry_id}/quote?quote_id=${q.id}`} style={{ textDecoration: "none" }}>
                  {card}
                </Link>
              );
            })}
